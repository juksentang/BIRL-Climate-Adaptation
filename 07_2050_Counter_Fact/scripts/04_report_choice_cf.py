"""Stage 4 (choice-model counterfactual): tables and figures.

Reads the outputs of 03_choice_counterfactual.py
  results/choice_cf/metrics.parquet      (climate, policy, country, draw, metric, value)
  results/choice_cf/crop_shares.parquet  (climate, policy, country, draw, action, share)
  results/choice_cf/run_info.json
and writes
  results/choice_cf/tables/{metrics_summary, climate_loss, policy_effects,
                            headline_ratio, ppc_crop_shares, crop_shifts}.csv
  results/choice_cf/figures/{fig_headline_ratio, fig_switch_ssp585,
                            fig_downside, fig_cv_pct, fig_crop_shift_heatmap}.pdf

Every interval is an 89% highest-density interval over posterior draws; draws are
paired across scenarios by their index (same thinned posterior draw k).

Conventions assumed from the engine (see CHOICE_CF_SPEC.md):
  - cv_log / cv_pct are relative to (baseline, none) already; within-climate
    policy effects on welfare are therefore differences of cv_log.
  - switch_share is the L1/2 distance of crop shares to (same climate, none);
    for policy == none it is 0 (or absent) and is not reported.
  - cost is the public cost per plot-season in USD; contraction has cost 0
    (per-dollar effects are NaN there).

Usage
  python3 scripts/04_report_choice_cf.py                 # real results
  python3 scripts/04_report_choice_cf.py --toy           # synthetic run in the scratchpad
  python3 scripts/04_report_choice_cf.py --results DIR --out DIR
"""
...
FORMAL = STEP07.parent
RESULTS_DEFAULT = STEP07 / "results" / "choice_cf"
ACTION_CFG = FORMAL / "06_BIRL_MCMC" / "data" / "action_space_config.json"
BIRL_SAMPLE = FORMAL / "06_BIRL_MCMC" / "data" / "birl_sample.parquet"
...
# ── helpers ──
def hpdi(x, mass=0.89):
    s = np.sort(np.asarray(x, float)); s = s[~np.isnan(s)]; n = s.size
    if n == 0:
        return float("nan"), float("nan")
    if n == 1:
        return float(s[0]), float(s[0])
    k = min(max(int(np.floor(mass * n)), 1), n - 1)
    w = s[k:] - s[: n - k]; i = int(np.argmin(w))
