"""NUTS estimation of the semi-parametric structural crop-choice model.

  V_ia   = a_c * mu_ia + b_c * sigma_ia + c_c * sigma_ia^2
  mu     = log1p(q50),  sigma = clip((log1p(q90) - log1p(q10)) / (2 * 1.2816), 0.01, 5)
...
summary.csv (median, 89% HPDI, r_hat, ESS), derived.csv (marginal dispersion
aversion at sigma 0.5 / 1.0, sigma* = -b/(2c)), ppc_action_freq.csv, run.log,
posterior.npz, results.json.
Run from 08_BIRL_v2/:  python3 slurm/run_semipar.py [--warmup W --samples S --chains K --skip-timing]
"""
...
RUN_DIR = OUT_DIR / "semipar"; RUN_DIR.mkdir(parents=True, exist_ok=True)
...
if TOY:
    rng = np.random.default_rng(0); Nt, At, C = 400, 6, 3
    countries = ["A", "B", "C"]; labels = [f"crop{i//3}_{['low','medium','high'][i%3]}" for i in range(At)]
...
sig_np = np.asarray(SIG); mask_np = np.asarray(MASK)
...
def model(null=False):
    with plate("countries", C):
        if null:
            ...
            a = sample("a_c", dist.Normal(0.0, 3.0)); b = sample("b_c", dist.Normal(0.0, 3.0)); c = sample("c_c", dist.Normal(0.0, 3.0))
...
for i, cn in enumerate(countries):
    ...
    drows.append(d)
der = pd.DataFrame(drows); der.to_csv(RUN_DIR / "derived.csv", index=False)
...
# ── [4c] PPC ──
def probs_at(a_, b_, c_, asc_):
    V = per_obs(a_)[:, None] * MU + per_obs(b_)[:, None] * SIG + per_obs(c_)[:, None] * SIG2
    alpha = per_obs(jnp.concatenate([jnp.zeros((C, 1)), asc_], axis=1))
    return jax.nn.softmax(jnp.where(MASK, center_reward(V, MASK) + alpha, INFEASIBLE_LOGIT), axis=-1)


probs_jit = jax.jit(probs_at)
...
top = pp.sort_values("observed", ascending=False).head(6)
for _, r in top.iterrows():
    say(f"  {r['label']:22s} obs {r['observed']:.3f} pred {r['predicted']:.3f}")

# ── [3] ASC-only null log-lik ──
def fit_null():
    guide = AutoMultivariateNormal(lambda: model(null=True), init_loc_fn=numpyro.infer.init_to_median())
    svi = SVI(lambda: model(null=True), guide, numpyro.optim.Adam(1e-2), Trace_ELBO())
    res = svi.run(jax.random.PRNGKey(0), 3000 if not TOY else 50, progress_bar=False)
    med = guide.median(res.params)
    _, tr = log_density(numpyro.handlers.substitute(lambda: model(null=True), data=med), (), {}, {})
    return float(tr["obs_action"]["fn"].log_prob(tr["obs_action"]["value"]).sum())
