...
outputs/exp_utility/exp_utility_log.json.
...
def model(family, asc=True):
    with plate("countries", C):
        ...
        th = sample("th_c", dist.Normal(0.0, 3.0)) if family in ("logcrra", "logmsd") else None
        if family == "logfree":
            wf = sample("wfree_c", dist.Normal(0.0, 2.0).expand([3]).to_event(1))       # (C,3): a, b, c
    ...
    if family == "logmed":
        ...
        rho = deterministic("rho_c", 1.0 + th)
        V = MU - 0.5 * per_obs(th)[:, None] * SIG2
    elif family == "logmsd":
        deterministic("theta_c", th)
        V = MU - per_obs(th)[:, None] * SIG
    elif family == "logfree":
        deterministic("w_c", wf)
        w = per_obs(wf)                                                                  # (N,3)
...
R = {}
for fam in ("logmed", "logev", "logcrra", "logmsd", "logfree"):
    R[fam] = fit(fam)
R["logcrra_noasc"] = fit("logcrra", asc=False)
...
for fam, r in R.items():
    ...
    d = r["det"]
    if "rho_c" in d:
        par = "rho=" + " ".join(f"{x:.2f}" for x in d["rho_c"])
    elif "theta_c" in d:
        par = "theta=" + " ".join(f"{x:.2f}" for x in d["theta_c"])
    elif "w_c" in d:
        par = "(a mu, b sig, c sig2)=" + " | ".join(" ".join(f"{x:+.2f}" for x in row) for row in d["w_c"])
    else:
        par = ""
...
o = OUT_DIR / "exp_utility_log"; o.mkdir(parents=True, exist_ok=True)
json.dump({"countries": countries, "N": N, "steps": STEPS, "results": R}, open(o / "exp_utility_log.json", "w"), indent=2)
say(f"written {o/'exp_utility_log.json'}")
