...
if [ -n "${AFTER}" ]; then
    ...
    J0=$(bash slurm/sb.sh --parsable slurm/00_smoke.sbatch)
    IDS+=("00_smoke ${J0}"); DEP="--dependency=afterok:${J0}"
    echo "00_smoke        -> job ${J0}"
    if [ "${TIMING}" = 1 ]; then
        J0B=$(bash slurm/sb.sh --parsable slurm/00b_timing.sbatch)
...
if [ "${SMOKE_ONLY}" = 0 ]; then
    for name in ${ONLY//,/ }; do
        ...
        J=$(bash slurm/sb.sh --parsable ${HOLD:+-H} ${DEP} "slurm/${name}.sbatch")
