...
Target: Rorqual (Calcul Québec / Alliance Canada), account `<account>`,
user `<user>`, project root `$SCRATCH/birl_v2`. Every value below was
...
Every sbatch: `--account=<account>`, one `--gres` as above, `--nodes=1`,
`--cpus-per-task=4` (8 for the main runs), `--mem=32G` (64G main),
`--output=$SCRATCH/birl_v2/08_BIRL_v2/outputs/slurm_logs/%x-%j.out` (a literal path: Slurm opens it before the
...
bash slurm/sync_up.sh              # 1. code + data -> $SCRATCH/birl_v2   (~2 min, 103 MB data)
...
* Accounts: `<account>`, `<account>_cpu` (QOS normal, MaxSubmit 1000). `$SCRATCH` = `$SCRATCH` (19 T free). `DependencyParameters=kill_invalid_depend`.
