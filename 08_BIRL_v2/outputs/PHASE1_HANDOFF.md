...
包路径：`08_BIRL_v2/`（本地）↔ `$SCRATCH/birl_v2/08_BIRL_v2/`（Rorqual）
...
所有 sbatch 共有：`--account=<account>`、`module load StdEnv/2023 python/3.11 cuda/12.9 cudnn/9.13.1.26 arrow/25.0.0`、venv 激活、`export BIRL_HOST_DEVICES=1`、开跑前 `assert jax.devices()[0].platform == 'gpu'`。软件栈：jax = jaxlib = jax_cuda12_plugin = jax_cuda12_pjrt = 0.10.2，numpyro 0.21.0，numpy 2.4.2，pandas 2.3.3，pyarrow 来自 arrow 模块。
...
ssh rorqual 'cd $SCRATCH/birl_v2/08_BIRL_v2 && tail -n 30 outputs/slurm_logs/00_smoke-21444911.out'
ssh rorqual 'cd $SCRATCH/birl_v2/08_BIRL_v2 && grep -h "^TIMING" outputs/v2_country_timing/run.log; cat outputs/v2_country_timing/timing.json'
...
ssh rorqual 'cd $SCRATCH/birl_v2/08_BIRL_v2 && sbatch slurm/02_main.sbatch'
# 或
...
Rorqual
  $SCRATCH/birl_v2/08_BIRL_v2/outputs/slurm_logs/<name>-<jobid>.out
  $SCRATCH/birl_v2/08_BIRL_v2/outputs/slurm_logs/jobids.txt        两次提交的 job id
  $SCRATCH/birl_v2/08_BIRL_v2/outputs/v2_country_timing/timing.json
  $SCRATCH/birl_v2/08_BIRL_v2/outputs/recovery/{svi,nuts}/report.md
  $SCRATCH/birl_v2/08_BIRL_v2/outputs/{v2_country,v2_country_gfix,v2_country_smax08}/
  $SCRATCH/birl_v2/06_BIRL_MCMC/data/   三个 06 数据文件（103 MB）
  $SCRATCH/birl_v2/venv                 setup_venv.sh 建好的环境
```
